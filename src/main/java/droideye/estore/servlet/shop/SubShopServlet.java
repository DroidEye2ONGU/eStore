...
@WebServlet(name = "SubShopServlet", urlPatterns = "/SubShopServlet")
public class SubShopServlet extends HttpServlet {
    @Override
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //判断图书数量是否是1,如果是1则进入删除Servlet处理
        if (bookNum == 1) {
            request.getRequestDispatcher("/ShopcartDeleteServlet?bookId=" + bookId).forward(request, response);
            return;
        }
